...
#### 1. Business purpose  
**1.1 Project Backgound**<br/>
최근 1인 가구의 급격한 증가와 현대인의 바쁜 일상으로 인해 식재료를 체계적으로 관리하는 것이 갈수록 어려워지고 있다. 대부분의 사람들이 냉장고 깊숙이 보관된 식재료의 존재를 잊어버리거나 구매한 날짜와 유통기한을 일일이 기억하지 못해, 결국 신선도가 떨어진 음식을 폐기하게 되며 경제적 손실과 환경 오염 문제가 발생할 수 있는 상황이라는 것을 알게 되었습니다. 주변에 자취를 하는 친구들 또한 어떤 재료가 있는지 일일히 확인하기 귀찮아하기에 한 번의 터치와 입력으로 버리기 전 먼저 사용할 수 있게 안내해주는 것이 있으면 편리할 것 같다라는 생각이 들었습니다. 이런 상황들과 생각들을 통해 이 프로젝트로 사용자가 구매한 식재료의 이름, 수량, 유통기한, 그리고 냉장 및 냉동 등의 보관 위치 정보를 디지털 데이터로 기록하고, 앱 실행 시마다 현재 날짜를 기준으로 유통기한 임박 및 경과 여부를 자동 판별하여 시각적 알림을 제공함으로써 식재료 폐기율을 최소화할 수 있는 직관적인 냉장고 관리 시스템을 생각해보게 되었습니다.

**1.2 Goal**
- 식재료의 이름, 수량, 유통기한 및 보관 위치(냉장/냉동)를 체계적으로 데이터화하여 관리한다.
- 검색 및 필터 기능을 통해 중복 구매를 방지하고 식재료 소비 효율을 극대화시킨다.

**1.3 Target Market**
- 자취생이나 1인 가구처럼 식재료 관리에 서툰 사용자들
- 식재료 폐기를 줄여 환경 보호나 가계 경제 절약을 하고 싶은 사용자들
- 남는 재료 없이 다양하게 사용해서 재료를 처리하고 싶은 사용자들
